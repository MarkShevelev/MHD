...
pub fn lxf_flux_f32 (f: &mut Uf32, u: &Uf32View, c0: f32, d: f32)
{
  ...
  for (((rho, mnt), bz), ((f_rho, f_mnt), f_bz) ) in inputs.zip(outputs)
  {
    ...
    *f_rho = (left_rho_f + right_rho_f) / 2.0f32 - (rho[1] - rho[0]) / (2.0f32 * d);
    ...
    let f_mnt_right = 
      ...
      bz[1] * bz[1] * 0.5 + 
      mnt[1] * mnt[1] / rho[1];
    *f_mnt = (f_mnt_left + f_mnt_right) / 2.0f32 - (mnt[1] - mnt[0]) / (2.0f32 * d);

    let f_bz_left  = bz[0] * mnt[0] / rho[0];
    let f_bz_right = bz[1] * mnt[1] / rho[1];
    *f_bz = (f_bz_left + f_bz_right) / 2.0f32 - (bz[1] - bz[0]) / (2.0f32 * d);
  }
}
